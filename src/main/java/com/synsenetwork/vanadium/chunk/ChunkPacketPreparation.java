...
public final class ChunkPacketPreparation implements AutoCloseable {
    private static final int MIN_PARALLEL_CHUNKS = 4;
    private static final int MAX_PREPARED_CHUNKS = 64;
    ...
    public static void prepare(List<LevelChunk> chunks) {
        ...
        if (batch == null || !Vanadium.config.enabled || !Vanadium.config.parallelChunkPackets
                || chunks.size() < MIN_PARALLEL_CHUNKS) return;
        // C2ME may enlarge batches; cap retained buffers and let later chunks use the vanilla path.
        int count = Math.min(MAX_PREPARED_CHUNKS, chunks.size());
        byte[][] payloads = new byte[count][];
        List<Runnable> tasks = new ArrayList<>(count);
        for (int i = 0; i < count; i++) {
            int index = i;
            LevelChunk chunk = chunks.get(i);
            tasks.add(() -> payloads[index] = serializeSections(chunk));
        }
        Vanadium.scheduler.prepare(tasks);
        for (int i = 0; i < count; i++) batch.put(chunks.get(i), payloads[i]);
    }

    private static byte[] serializeSections(LevelChunk chunk) {
        int size = 0;
